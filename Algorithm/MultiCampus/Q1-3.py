...
def makeNormalData(saleKind, saleNumber): # 판매된 물품 코드 문자열 배열, 판매된 물품의 개수
    ...
    ################ 여기부터 코딩 (1) -------->
    removelist = list(' 0123456789#$@&*%')
    for item in saleKind:
        item_remove = ''
        for c in item:
            if c not in removelist:
                item_remove += c.lower()
        retData.append(item_remove)
...
def correctLetterOrder(saleKind, saleNumber, products): # 판매된 물품 코드 문자열 배열, 판매된 물품의 개수, 편의점에서 판매하는 물품 코드 배열 (13종 고정)
    ...
    for product in products:
        chrs = list(product)
        chrs.sort()
        product_sort = ''.join(chrs)
        products_sort.append(product_sort)

    for item in saleKind:
        chrs = list(item)
        chrs.sort()
        item_sort = ''.join(chrs)
        for i in range(len(products_sort)):
            if item_sort == products_sort[i]:
                retData.append(products[i])
                break
